...
''' MIDI <-> JNN converter for creating JamNet datasets and playing JamNet outputs. '''
...
def midi_to_jnn(full_raw_midi_dir):
    ...
    jnn_piece = grouped_tracks_to_jnn(grouped_tracks, midif)  # TODO: (1) todo.txt
...
def parse_midi_track_instrument(track):
    for msg in track:
        if msg.type == 'program_change':
            for type in INSTRUMENTS:
                for midi_index in INSTRUMENTS[type]:
...
def jnn_to_midi(jnn_piece, full_midi_save_dir):
    ...
    for channel, jnn_instrument in enumerate(jnn_piece):
        if not jnn_piece[jnn_instrument][0] is None:
            midif.tracks.append(jnn_track_to_mido_track(jnn_piece[jnn_instrument], jnn_instrument, channel))
...
def jnn_track_to_mido_track(jnn_track, jnn_instrument, channel):
    messages = []
    messages.append(Message(type='program_change', channel=channel, program=JNN_INSTRUMENTS[jnn_instrument], time=0))
    messages.append(MetaMessage("track_name", name=jnn_instrument, time=0))
...
# def tempo_normalize(jnn_piece):
#    tempo = jnn_piece.pop("tempo")
#    for t in jnn_piece:
#        for
#
#
# def tempo_denormalize(jnn_piece):
#